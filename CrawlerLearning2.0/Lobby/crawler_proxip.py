...
from MyTool.RequestTool import RequestTool
from MyTool.fileWriter  import  writer
__author__ = '田小饼'
...
def xc_iplist():
    ...
    iplist = []
    res = requests.get(url=proxyIPsource_url, headers=RequestTool.get_headers())
...
def kdl_iplist(url):
    ...
    iplist=[]
    res=requests.get(url,headers=RequestTool().get_headers())
    soup=BeautifulSoup(res.text,"html.parser")
...
def clean_ip(ip_list):
    ...
    writer().list2txt(filename="proxyIPpool", data=cleaned_iplist)
...
if __name__ == "__main__":
    ...
    #thlt(clean_ip, kdl_iplist())
    num=1

    while 1:
        url = "https://www.kuaidaili.com/free/inha/" + str(num)
        thlt(clean_ip, kdl_iplist(url))
        num+=1
