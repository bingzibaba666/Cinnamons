...
def dieniao_iplist():
    ...
    res_list = [(x1 + ":" + x2) for x1, x2 in zip(addr_list, ip_list)]
    return res_list
...
def clean_ip(ip_list):
    ...
    with open("./storage/ippool.txt",'a',encoding='UTF-8') as  f :
        # f.write(cleaned_iplist)
        for item in  cleaned_iplist:
            f.write(item+"\n")
